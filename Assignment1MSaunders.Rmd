...
read_data_with_type <- function(type, file_object){
  years <- 2017:2020
  quarters <- 1:4
  general_file_path <- "../Documents/Fall Quarter 2020/Shaping the Future of the Bay/PG&E_Data/"
  file_path <-
    paste0(
      general_file_path,
      type,
...
```
```{r}
plotly_residential_critical_months <- plot_ly() %>%
  add_trace(
    data = residential_critical_months %>% filter(CUSTOMERCLASS == "Elec- Residential"),
    x = ~YEAR %>% factor(),
    y = ~February,
    type = "bar",
    name = "Electric"
  ...
  add_trace(
    data = residential_critical_months %>% filter(CUSTOMERCLASS == "Gas- Residential"),
    x = ~YEAR %>% factor(),
    y = ~February,
    type = "bar",
    name = "Gas"
  ...
  add_trace(
    data = pge_commercial_final %>% filter(CUSTOMERCLASS == "Elec- Residential"),
    x = ~YEAR %>% factor(),
    y = ~March,
    type = "bar",
    name = "Electric"
  ...
  layout(
    ...
    title = "Residential Energy Usage in Critical Months (PG&E)",
    barmode = "stack",
